...
1. Расчёт скоростей реакции: `wᵢ = |ΔC_A|/Δt`
2. Линеаризацию: `ln(w) = ln(k) + n·ln(C_A)`
3. Регрессионный анализ (МНК) для определения `n` и `k`
4. Симуляцию концентраций всех компонентов по времени
