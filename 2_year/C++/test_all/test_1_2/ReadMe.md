...
```mermaid
graph TD
    A[Начало] --> B[Получить step, min, max]
    B --> C[Очистить chart]
    C --> D[Добавить series: part_one, part_two, part_three]
    D --> E[Инициализировать x = min]
    E --> F{x ≤ max?}
    F -->|Да| G{abs(x) ≤ 1?}
    G -->|Да| H[y = x²]
    H --> I[Добавить точку в part_one]
    I --> J
    G -->|Нет| K[y = -1/x]
    K --> L[Добавить точку в part_two]
    L --> J[Увеличить x на step]
    J --> F
    F -->|Нет| M[Конец]
```

#### Пример работы программы
![Демонстрация построения графика](graphic.gif)
...
![Визуализация сортировки выбором](sort_visual.gif)
...
#### Пример работы программы
![Демонстрация сортировки](sort.gif)
...
#### Пример работы программы
![Демонстрация поиска слов](kr2.gif)
