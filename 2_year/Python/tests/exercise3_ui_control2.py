# exercise3_ui_control2.py

import curses
import time

def draw_formula(stdscr, h, w):
    """Отрисовка формулы для упражнения 3"""
...
def run_exercise3_ui_control2(stdscr, control_obj):
    curses.curs_set(0)
    ...
    while True:
        ...
        if key == 27:
            ...
            try:
                a = float(inputs["a"]) if inputs["a"] != "" else -2.0
                b = float(inputs["b"]) if inputs["b"] != "" else 2.0
                h_val = float(inputs["h"]) if inputs["h"] != "" else 0.1

                result_list, error = control_obj.exercise_3(a, b, h_val)
